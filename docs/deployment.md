...
# 应用到线上（0016 错题本 + 0017 邀请码必须都应用，否则错题加载失败 / 邀请码功能 500）
...
- `JOB_SECRET`：日报发行与投递内部任务密钥，也用于后台生成 AI 解析邀请码（`/api/invite/admin/*`）。**必填**，不得暴露给浏览器。
...
```sh
cd apps/web
$env:PUBLIC_API_BASE = "https://api.example.com"   # 你的 Worker 域名
npx astro build
