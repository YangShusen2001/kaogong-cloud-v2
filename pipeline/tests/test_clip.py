...
def test_extract_paragraphs_unescapes_indent_entities():
    html = (
        "<html><body>"
        '<div id="content">'
        + "<p>&emsp;&emsp;" + "正文第一段，" * 8 + "</p>"
        + "<p>&nbsp;记者&nbsp;" + "正文第二段，" * 8 + "</p>"
        + "</div></body></html>"
    )
    paras, _method = extract_paragraphs(html)
    assert len(paras) == 2
    assert "&emsp;" not in paras[0]
    assert "&nbsp;" not in paras[1]
    assert paras[0].startswith("正文第一段")
    assert paras[1].startswith("记者")
