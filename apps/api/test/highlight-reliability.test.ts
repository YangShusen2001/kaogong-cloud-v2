...
function paragraphBody(baseVersion: number, text = "可靠划线") {
  return {
    ...
    paragraphIndex: 2,
    baseVersion,
    spans: [{ text, note: "备注", explanation: "", styles: ["green", "underline"], start: 1, end: 5 }],
  };
}
...
describe("highlight paragraph reliability", () => {
  ...
  it("removes superseded legacy rows for the same paragraph after a successful versioned save", async () => {
    const { app, sqlite } = makeContext();
    insertLegacyHighlight(sqlite, 2, "legacy-cleanup-target");

    const response = await app.request("/api/highlights/paragraph", json("PUT", paragraphBody(0), "legacy-owner"));

    expect(response.status).toBe(200);
    expect(sqlite.prepare("SELECT count(*) AS count FROM highlights WHERE owner_id = ? AND article_id = ? AND paragraph_index = ?")
      .get("device:legacy-owner", ARTICLE_ID, 2)).toEqual({ count: 0 });
    const saved = sqlite.prepare("SELECT count(*) AS count FROM highlight_paragraphs WHERE owner_id = ? AND article_id = ? AND paragraph_index = ?")
      .get("device:legacy-owner", ARTICLE_ID, 2);
    expect(saved).toEqual({ count: 1 });
  });

  it("keeps legacy rows for other paragraphs when saving a different paragraph", async () => {
    const { app, sqlite } = makeContext();
    insertLegacyHighlight(sqlite, 2, "legacy-cleanup-target");
    insertLegacyHighlight(sqlite, 3, "legacy-other-paragraph");

    const response = await app.request("/api/highlights/paragraph", json("PUT", paragraphBody(0), "legacy-owner"));

    expect(response.status).toBe(200);
    expect(sqlite.prepare("SELECT id FROM highlights ORDER BY id").all())
      .toEqual([{ id: "legacy-other-paragraph" }]);
  });

  it("keeps legacy rows when the versioned save conflicts", async () => {
    const { app, sqlite } = makeContext();
    await app.request("/api/highlights/paragraph", json("PUT", paragraphBody(0), "legacy-owner"));
    insertLegacyHighlight(sqlite, 2, "legacy-conflict-target");

    const conflict = await app.request("/api/highlights/paragraph", json("PUT", paragraphBody(0, "陈旧版本"), "legacy-owner"));

    expect(conflict.status).toBe(409);
    expect(sqlite.prepare("SELECT id FROM highlights WHERE id = ?").get("legacy-conflict-target"))
      .toEqual({ id: "legacy-conflict-target" });
  });
